...
TCanvas* c1;
TFile* datafile=NULL;
TFile* datafile2=NULL;
TColor *color[120];
int Palette[20];

TApplication   *theapp=NULL;
int graphic(1),makeps(1),makepdf(1),drawtotarea(1), nocol(0), doNP(0), doMFV(0);
...
string plotname4 = "";
string dirname4 = "";
string gamma_use_root = "";

TString season,addtxt="",addtxt2="",addtxt3="";

TString xlab,ylab;
int col1,col2,logoposition;
int col3,col4,col5,col6, col7,col8;
int smooth(0);
...
void tria(int vubd, int dmdd, int epsd, int dmsd, int s2bd, int c2bd, 
	  ...
	  int btovgd, int D0p0d, int kl2p0nnd, int BtoKppd, string);
...
void cos2b_areas(double,double,int, string);
void draw_cos2b(double bound, int color);
// porcata?
void D0p0_areas(double beta_min, double beta_max, int color, string side);
...
TObjArray* GraphFromHisto(TH2D*, double, double);
void drawFromGraph(TObjArray* contour, int ind, string DrawOpts, int col);
...
void get_data(const char* HistoName, const char* DirName,int DrawOpt, int filenum = 1);
...
void drawfunc(double rho[400], double eta[400], int nsind, string);
void drawfunc200(double rho[200], double eta[200], int nsind, string);
void setdrawlevel(TH2D* Constrain, double level[5], int sel[6], string);
void drawcont(TH2D* ThisConstrain, double level[5],int nlevel, int color, string);
...
double CalcLevel(TH1D* Constraint, double level, string side);
double CalcLevel_Mediana(TH1D* Constraint, double level, string side);
